...
import { money, jalaliDate } from "@/lib/format";
import { exportCsv } from "@/lib/csv";
import { BookOpen } from "lucide-react";
import { getLocalPayablesDetails } from "@/lib/local-store";
...
export const Route = createFileRoute("/reports/payables")({
  ...
  errorComponent: makeRouteErrorComponent(t.payables),
});
...
function PAY() {
  const { data, isFetching } = useQuery({
    queryKey: ["payables"],
    queryFn: (): Promise<Row[]> => getLocalPayablesDetails(),
  });
  ...
  return (
    <ReportShell
      ...
      onExport={() =>
        exportCsv(
          `payables-${new Date().toISOString().slice(0, 10)}`,
          [
            { key: "name", header: "عرضه کوونکی" },
            { key: "phone", header: "شمېره" },
            { key: "total_purchases", header: t.totalPurchases },
      ...
      {isFetching && rows.length === 0 ? (
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-5">
            <StatCard label={t.payablesTotal} value={money(totals.balance)} tone="destructive" />
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/40">
                  <tr>
                    ...
                    <th className="p-2 text-left">{t.totalPurchases}</th>
                  ...
                  {rows.map((r) => {
                    ...
                    return (
                      <tr key={r.supplier_id} className="border-t">
                        ...
                        <td className="p-2 text-left font-mono">{money(r.total_purchases)}</td>
                        ...
                        <td className="p-2">
                          <Button asChild size="sm" variant="ghost">
                            <Link to="/suppliers/$id/ledger" params={{ id: r.supplier_id }}>
